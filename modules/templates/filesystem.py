...
class TemplateLoader(jinja2.BaseLoader):
    def get_source(self, environment, template):
        template_path = pkg_resources.resource_filename(__name__, os.path.join('../../templates', '%s.tmpl' % template.replace('.', '/')))
...
def TemplateFactory(xtmpl_name, globals=None):
    try:
        return mod.Environment(TemplateLoader()).get_template(xtmpl_name, globals=globals)
