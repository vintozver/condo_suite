...
class Handler(_Handler):
    ...
    def process_authentication(self):
        ...
        with mod_mongo.DbSessionController() as db_session:
            user_doc = db_session[config.name]['users'].find_one({'ext.google.email': google_email}, {'_id': True})
    ...
    def get_stored_credentials(self, purpose):
        ...
        if credentials is not None:
            return oauth2client.client.OAuth2Credentials(credentials.get('access_token'), config.google_client_id, config.google_client_secret, credentials.get('refresh_token'),
    ...
    def store_credentials(self, purpose, credentials):
        ...
        with mod_mongo.DbSessionController() as db_session:
            db_session[config.name]['sessions'].update_one({'_id': req_session.id}, {'$set': {('data.ext.google.credentials.%s' % purpose): db_doc}})
    ...
    def redirect_authorization(self, purpose, return_url):
        flow = oauth2client.client.OAuth2WebServerFlow(config.google_client_id, config.google_client_secret,
    ...
    def process_code(self, code, purpose, return_url):
        flow = oauth2client.client.OAuth2WebServerFlow(config.google_client_id, config.google_client_secret, self.calc_perms(purpose), config.google_redirect_uri, auth_uri='https://accounts.google.com/o/oauth2/auth', token_uri='https://accounts.google.com/o/oauth2/token')
        try:
