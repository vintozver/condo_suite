...
class Handler(_Handler):
    ROUTE_MAP = [
        ...
        {'regex': re.compile(r'^/\bapi\b'), 'map': [
            {'regex': re.compile(r'^/parking/event/?$'), 'handler': 'handlers.web.api_parking_event', 'params': {'action': 'search'}},
            {'regex': re.compile(r'^/parking/event/([0123456789abcdefABCDEF]+)/file/([0123456789abcdefABCDEF]+)/?$'), 'handler': 'handlers.web.api_parking_event', 'params': {'action': 'file', 'oid': lambda rex: rex.group(1), 'file_oid': lambda rex: rex.group(2)}},
            {'regex': re.compile(r'^/parking/event/([0123456789abcdefABCDEF]+)/?$'), 'handler': 'handlers.web.api_parking_event', 'params': {'action': 'event', 'oid': lambda rex: rex.group(1)}},
        ]},
