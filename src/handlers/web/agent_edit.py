...
class Handler(util.handler.Handler):
    ...
    def view(self):
        ...
        def query_agent_list():
            with mod_mongo.DbSessionController() as db_session:
                for db_doc in db_session[config.name]['agent'].find({'closed': {'$exists': False}}):
    ...
    def process_create(cls, name, address):
        ...
        if address:
            ...
            del agent_address
        with mod_mongo.DbSessionController() as db_session:
            return db_session[config.name]['agent'].insert_one(agent).inserted_id
    ...
    def process_update(cls, id_agent, name, address):
        ...
        if address:
            ...
            del agent_address
        with mod_mongo.DbSessionController() as db_session:
            return db_session[config.name]['agent'].update_one({'_id': id_agent}, {'$set': agent}).modified_count > 0

    @classmethod
    def process_close(cls, id_agent):
        with mod_mongo.DbSessionController() as db_session:
            return bool(db_session[config.name]['agent'].update_one({'_id': id_agent, 'closed': {'$exists': False}}, {'$set': {'closed': datetime.datetime.utcnow()}}).modified_count)

    @classmethod
    def process_get(cls, id_agent):
        with mod_mongo.DbSessionController() as db_session:
            agent = db_session[config.name]['agent'].find_one({'_id': id_agent, 'closed': {'$exists': False}})
            if agent is None:
