...
class Handler(_Handler):
    def autocomplete_search(self, token):
        ...
        with mod_mongo.DbSessionController() as db_session:
            db_documents = db_session[config.name]['users'].find({'name': {'$regex': regex}})
    ...
    def autocomplete_search_agent(self, token, user_id):
        ...
        with mod_mongo.DbSessionController() as db_session:
            db_documents = db_session[config.name]['agent'].find({'name': {'$regex': regex}, 'closed': {'$exists': False}})

            result = list()
            for db_document in db_documents:
                agent_id = mod_mongo.bson.objectid.ObjectId(db_document['_id'])

                db_user = db_session[config.name]['users'].find_one({'_id': user_id}, {'agents': {'$elemMatch': {'_id': agent_id}}})
