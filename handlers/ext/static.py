...
class Handler(util.handler.Handler):
    ...
    def __call__(self, path):
        ...
        modified = datetime.datetime.utcfromtimestamp(st.st_mtime).replace(microsecond=0)  # RFC formats don't suppport microseconds
        c_len = st.st_size
        ...
        if since:
            since = email.utils.parsedate(since)
        if since:
            since = datetime.datetime(*since[:6])
        ...
        self.req.setHeader('Last-Modified', modified.strftime('%a, %d %b %Y %H:%M:%S GMT'))
        self.req.setHeader('ETag', etag)
